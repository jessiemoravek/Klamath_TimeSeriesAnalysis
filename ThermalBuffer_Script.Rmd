...
```{r}
readRDS("May.rds")
data_August <- daily_means_condensed %>%filter(day>'2020-08-01'& day < '2020-08-31')
May_August <- data_August[,c("May","day")]         
SC_August <- data_August[,c("SC","day")] 

png("Fig_MayPond.png", width = 700, height = 400)
ggplot()+
  geom_line(data = May_August, aes(x = day, y = May, color = "May Pond"))+
  geom_line(data = SC_August, aes(x = day, y = SC, color = "Seiad Creek"))+
  labs(x = "Date",
       y = "Temperature (C)")+
  theme_classic()+
