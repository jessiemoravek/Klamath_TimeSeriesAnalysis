...
AP1 <- AP1 %>% 
  group_by(date) %>% 
  summarize(Temp = mean(Temp))#Remove duplicates
missing_data <- AP1[!complete.cases(AP1),] 
missing_data

#Position dataset into a full year data frame, bin by 15 min steps
...
quarterHour <- as.data.frame(quarterHour) #convert "quarterHour" to data frame
AP1 <- left_join(quarterHour,AP1) #join quarterHour and dataset

#Position dataset into a full year data frame, bin by daily steps
...
quarterHour <- seq(mdy_hm('7/1/2020 00:00'),mdy_hm('7/13/2021 00:00'),by = "15 mins")
quarterHour <- as.data.frame(quarterHour) #convert "quarterHour" to data frame
AP2 <- left_join(quarterHour,AP2) #join quarterHour and dataset
...
quarterHour <- seq(mdy_hm('7/1/2020 00:00'),mdy_hm('7/13/2021 00:00'),by = "15 mins")
quarterHour <- as.data.frame(quarterHour) #convert "quarterHour" to data frame
AP3 <- left_join(quarterHour,AP3) #join quarterHour and dataset
...
quarterHour <- seq(mdy_hm('7/1/2020 00:00'),mdy_hm('7/13/2021 00:00'),by = "15 mins")
quarterHour <- as.data.frame(quarterHour)#convert "quarterHour" to data frame
APck <- left_join(quarterHour,APck)#join quarterHour and dataset
...
UL1 <- UL1 %>% 
  group_by(date) %>% 
  summarize(Temp = mean(Temp))#Remove duplicates
missing_data <- UL1[!complete.cases(UL1),] 
missing_data #Some missing data
#need to take average of surrounding 2 measurements (2021-01-26 14:45:00), which is row 19504
##The two rows surrounding 2021-01-26 14:45:00 are 14:30 and 15:00. Both of these temperatures are 7.89. 
UL1[19504,2] <- mean(7.89,7.89)

#need to remove (2020-07-27 15:58:00), which is row 1938
#need to remove (2020-12-26 14:50:00), which is row 16527
#need to remove (2021-01-26 14:44:00), which is row 19503
#need to remove (2021-01-26 14:46:00), which is row 19505
UL1 <- UL1[c(1:1937,1939:16526,16528:19502,19504,19506:35510),]
missing_data <- UL1[!complete.cases(UL1),] 
missing_data #no missing data
...
#Position dataset into a full year data by 15 min steps
...
KSV <- read.csv("SeiadValley_KlamathMain_QAQCdata.csv")
KSV$date <- lubridate::mdy_hm(KSV$TimeStamp)#convert dates to POSIXct format 
...
hour <- seq(mdy_h('05/02/2016 00'),mdy_h('02/01/2022 23'),by = "hour") #Create an object that goes hour by hour for the entire time series 
...
KSV_ts <- ts(KSV_hourly$zTemp, start = c(123, 0), frequency = 24) # This time series starts on 2 May 2016 at 12 am, so it starts on day 123 (leap year) at hour 0 and the frequency is 24 (24 hours per day)
plot(KSV_ts)
```
...
KO <- read.csv("Orleans_KlamathMain_QAQCdata.csv")
KO$date <- lubridate::mdy_hm(KO$TimeStamp)#convert dates to POSIXct format 
tail(KO)
...
#Create hourly sequence to ensure all missing data is accounted for
hour <- seq(mdy_h('5/2/2016 00'),mdy_h('2/1/2022 23'),by = "hour") #Create an object that goes hour by hour for the entire time series
hour <- as.data.frame(hour) #convert "hour" to data frame
...
#Convert to time series
KO_ts <- ts(KO_hourly$zTemp, start = c(123, 0), frequency = 24) # This time series starts on 2 May 2016 at 2 am, so it starts on day 123 (leap year) at hour 15 and the frequency is 24 (24 hours per day)
```
...
KSV_hourly_2020 <- KSV_hourly[c(35785:47472),]
KO_hourly_2020 <- KO_hourly[c(35785:47472),]
...
#Percent interpolated values
NROW(na.omit(KSV_KO_2020$predicted))/NROW(KSV_KO_2020$predicted) 
...
```
##Bin KSV data by hour and day day and cut to timeframe of interest
```{r}
#Cut dataset to correct hourly timesteps 
hour <- seq(mdy_h('7/1/2020 00'),mdy_h('7/13/2021 00'),by = "hour")
hour <- as.data.frame(hour)
KSV_KO_2020 <- left_join(hour,KSV_KO_2020)
...
#Dataframe of 15 min water temperatures
```{r}
...
#need to remove AP3 (artificially hot)
```
...
```
#Corr, Mean, and SD between multiple TS in a pond
##Seiad Creek Ponds
```{r}
#SP 
#Check correlation between SP1 and SP2
corSP <- cor(daily_means$SP1, daily_means$SP2, use = "na.or.complete", method = "pearson")
...
#GP 
#Check correlation between SP1 and SP2
cor(daily_means$GP2, daily_means$GP3, use = "na.or.complete", method = "pearson")
...
#UL 
#Check correlation between SP1 and SP2
cor(daily_means$ULout, daily_means$UL3, use = "na.or.complete", method = "pearson")
...
#LL
#Ahh there's three what to do
cor(daily_means$LL1, daily_means$LL2, use = "na.or.complete", method = "pearson")
...
#SC
cor(daily_means$LSck, daily_means$MayCk, use = "na.or.complete", method = "pearson")
...
#HC
cor(daily_means$FGck, daily_means$ULck, use = "na.or.complete", method = "pearson")
...
SP <- daily_means %>% rowid_to_column(var = "day") %>% ggplot()+
  ...
  theme(text=element_text(size=16), legend.position="none")+
  geom_text(x=200, y=20, label="r = 0.999 \nmean difference = 0.255 \nsd diffence = 0.286")
...
GP <- daily_means %>% rowid_to_column(var = "day") %>% ggplot()+
  ...
  theme(text=element_text(size=16), legend.position="none")+
  geom_text(x=200, y=20, label="r = 0.979 \nmean difference = 0.134 \nsd diffence = 0.708")
...
UL <- daily_means %>% rowid_to_column(var = "day") %>% ggplot()+
  ...
  theme(text=element_text(size=16), legend.position="none")+
  geom_text(x=200, y=20, label="r = 0.991 \nmean difference = 0.812 \nsd diffence = 0.655")
...
LL <- daily_means %>% rowid_to_column(var = "day") %>% ggplot()+
  ...
  theme(text=element_text(size=16), legend.position="none")+
  geom_text(x=200, y=20, label="r = 0.988 \nmean difference = 0.006 \nsd diffence = 0.647")
...
```{r}
#Hourly
quarterHour_condensed <- cbind(AP = quarterHour_temps$AP1, SP = quarterHour_temps$SP, Durazo = quarterHour_temps$Durazo, LS = quarterHour_temps$LS, May = quarterHour_temps$May, FG = quarterHour_temps$FG3, GP = quarterHour_temps$GP, UL = quarterHour_temps$UL, LL = quarterHour_temps$LL, SC = quarterHour_temps$SC, HC = quarterHour_temps$HC, KSV = quarterHour_temps$KSV)
quarterHour_condensed <- as.matrix(t(quarterHour_condensed))

#Daily Mean
daily_means_condensed <- cbind(AP = daily_means$AP1, SP = daily_means$SP, Durazo = daily_means$Durazo, LS = daily_means$LS, May = daily_means$May, FG = daily_means$FG3, GP = daily_means$GP, UL = daily_means$UL, LL = daily_means$LL, SC = daily_means$SC, HC = daily_means$HC, KSV = daily_means$KSV)
daily_means_condensed <- as.matrix(t(daily_means_condensed))

#Daily Max
daily_maxs_condensed <- cbind(AP = daily_maxs$AP1, SP = daily_maxs$SP, Durazo = daily_maxs$Durazo, LS = daily_maxs$LS, May = daily_maxs$May, FG = daily_maxs$FG3, GP = daily_maxs$GP, UL = daily_maxs$UL, LL = daily_maxs$LL, SC = daily_maxs$SC, HC = daily_maxs$HC, KSV = daily_maxs$KSV)
daily_maxs_condensed <- as.matrix(t(daily_maxs_condensed))

#Daily Min
daily_mins_condensed <- cbind(AP = daily_mins$AP1, SP = daily_mins$SP, Durazo = daily_mins$Durazo, LS = daily_mins$LS, May = daily_mins$May, FG = daily_mins$FG3, GP = daily_mins$GP, UL = daily_mins$UL, LL = daily_mins$LL, SC = daily_mins$SC, HC = daily_mins$HC, KSV = daily_mins$KSV)
