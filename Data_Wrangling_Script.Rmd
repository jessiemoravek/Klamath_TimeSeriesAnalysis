...
AP1_daily <- AP1 %>% 
  group_by(day) %>% 
  summarize(mean_temp=mean(Temp, na.rm = T),max_temp=max(Temp, na.rm = T), min_temp=min(Temp), amp_temp=(max(Temp)-min(Temp)))
#Should we drop that day of data?? 
...
AP3_quarterHour <- as.data.frame(AP3$quarterHour)

library(janitor)
janitor::get_dupes(AP3_quarterHour)

AP3 %>% filter(quarterHour == '2020-11-01 01:00:00')
View(AP3)

summarize(temp = mean(temp))
n_dupes = n()
summarize(temp = mean(temp), n_dupes = n())
