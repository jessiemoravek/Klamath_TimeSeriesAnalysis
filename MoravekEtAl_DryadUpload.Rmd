...
##Did not include code for interpolating Klamath River temps at Seiad
##Can I include separate files for all the DO probes? Annoying and hard to combine them all. 
##Should I include the time lag testing in the MAR model code? 
##Can I use a separate dataset that I pulled specially for the May pond exampels? 
##Note: Wavelet figures used in final manuscript are produced without z-scored data to make the figure more interpretable. Which code should I include in this code here?
##FUnding sources smh
...
##Pond and creek temperature data were obtained via a series of 21 temperature sensors (HOBO MX2201) placed in  9 ponds and 2 creeks from July 2020-July 2021. Water temperature was measured every 15 minutes. 
...
quarterHour_condensed <- readRDS("quarterHour_condensed.rds") #Matrix of 15 min temps
daily_means_condensed <- readRDS("daily_means_condensed.rds") #Matrix of mean daily water temperatures for 9 ponds, 2 creeks, and mainstem Klamath from 1 July 2020 to 13 July 2021. "Condensed" means that all invalid sensors are removed from the dataset.
daily_maxs_condensed <- readRDS("daily_maxs_condensed.rds") #Matrix of daily maximum temps
daily_mins_condensed <- readRDS("daily_mins_condensed.rds") #Matrix of daily minimum temps


#AIR TEMPERATURE DATA
##Both air temperature datasets were downloaded from the National Oceanic and Atmospheric Administration’s Climate Data Online database for the Slater Butte air sensor in Siskiyou County, CA.

covariate <- readRDS("covariate.rds") #Matrix of mean daily air temperatures from 1 July 2020 to 13 July 2021
airtemp_lag <- read.csv("SlaterButteAirTemp_2020_2021_lag.csv") #Same air temperature dataset as above, but lagged by several days to test most appropriate time lag for MAR models  


#HISTORICAL POND TEMPERATURE DATA
##Historical temperature data from Alexander and Stender Ponds was provided by the Mid Klamath Watershed Council (MKWC). Temperature data were collected hourly in these ponds from 2010-2021 using HOBO U22 sensors. 

APH <- read.csv("Alexander_Historical_2.csv") #Historical data from Alexander Pond
SPH <- read.csv("Stender_Historical_2.csv") #Historical data from Stender Pond 
...
daily_means <- readRDS("daily_means.RDS") #Same as daily_means_condensed but in dataset format. This data set is not "condensed" meaning it still includes sensors that were ultimately removed from analysis. 
mod1_params <- readRDS("mod1_params.RDS") #Parameters from the best supported MAR model, used to make Figure 3c (air temperature effects)
May <- readRDS("May.rds") #Pulled May Pond data to make Figure 3a and 3b (May Pond example)
MayCk <- readRDS("MayCk.rds") #Pulled May Creek data to make Figure 3a and 3b (May Pond example)
...
#All subsequent models use t-1 lag in covariate dataset
```
...
#FIGURE 3: MAY POND EXAMPLES AND COVARIATE EFFECTS
##May Pond Example, August 2020
...
saveRDS(forecast_fit1,"forecast_fit1.RDS") #This step takes a long time to run, so saving intermediate results to facilitate figure creation later. 
...
y_inter_df <- readRDS("y_inter_df.rds") #Save as an intermediate data product
```
###Check for NAs
```{r}
...
APH.w <- analyze.wavelet(y_inter_df, "temp",
                        ...
                        date.format = "%Y-%m-%d-%h") 
str(APH.w)
saveRDS(APH.w,"APH.w.rds") #Note: very large file; save as an intermediate data product

```

###Wavelet Image ***Figure 5a
```{r}
#read in data as needed
##Note: APH.w saved locally, very large file
APH.w <- readRDS("APH.w.rds")

png("Fig_APH_Wavelet.png", width = 800, height = 500)
wt.image(APH.w, color.key = "quantile", n.levels = 100, main = "Alexander Pond Wavelet",  
         legend.params = list(lab = "wavelet power levels", mar = 6.5, lab.line=4, label.digits = 4), 
...
wt.avg(APH.w, siglvl = 0.05, sigcol = "red",
       periodlab = "period (days)")
```

###Pull 24 hr period ***Figure 5a
```{r}
#read in data as needed
...
saveRDS(forecast_fit2,"forecast_fit2.RDS") #This step takes a long time to run, so saving intermediate results to facilitate figure creation later. 
```
...
saveRDS(y_inter2_df,"y_inter2_df.rds") #Save as an intermediate data product
```
###Check for NAs
```{r}
...
SPH.w <- analyze.wavelet(y_inter2_df, "temp",
                        ...
                        date.format = "%Y-%m-%d-%h") 
str(SPH.w) # Output
saveRDS(SPH.w,"SPH.w.rds") #Save as an intermediate data product
```

###Wavelet Image ***Figure 5b
```{r}
#read in data as needed
...
wt.avg(SPH.w, siglvl = 0.05, sigcol = "red",
       periodlab = "period (days)")
```

###Pull 24 hr period ***Figure 5b
```{r}
#read in data as needed
...
png("Fig_RawTempSC.png",width = 1000, height = 600)
(AP + SP + D) / (LS + May + SC)
dev.off()

png("Fig_RawTempHC.png",width = 1000, height = 600)
...
##Correlation, Mean, and Standard Deviation between multiple temperature time series in a pond (only for ponds with more than 1 sensor)
```{r}
#Stender Pond (SP) 
#Check correlation between SP1 and SP2
corSP <- cor.test(daily_means$SP1, daily_means$SP2, use = "na.or.complete", method = "pearson")
#Tightly correlated-- 0.9987

#Check differences
diffSP <- daily_means$SP1 - daily_means$SP2
meanSP <- mean(diffSP,na.rm=T) #0.2552828
sdSP <- sd(diffSP,na.rm=T) #0.286226

#Goodman Pond (GP) 
#Check correlation between SP1 and SP2
cor.test(daily_means$GP2, daily_means$GP3, use = "na.or.complete", method = "pearson")
#Tightly correlated-- 0.979

#Check differences
diffGP <- daily_means$GP3 - daily_means$GP2
meanGP <- mean(diffGP,na.rm=T) #0.1340852
sdGP <- sd(diffGP,na.rm=T) #0.7083215

#Check correlation between SP1 and SP2
cor.test(daily_means$GP1, daily_means$GP2, use = "na.or.complete", method = "pearson")
# correlated-- 0.9850231  

#Check differences
diffGPa <- daily_means$GP2 - daily_means$GP1
meanGPa <- mean(diffGPa,na.rm=T) #-0.577712
sdGPa <- sd(diffGP,na.rm=T) #1.249114
