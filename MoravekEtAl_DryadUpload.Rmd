...
##Klamath River temperature data was collected by the Karuk Tribe and accessed with permission from the Karuk Tribe Water Quality Department (Accessed 27 September 2022). We used data between May 2020 and February 2021. We used a combination of data readings from the Seiad Valley station as well as interpolated data using a linear regression from the Orleans station when Seiad Valley data was unavailable (5.5% of Seiad Valley data was interpolated). Water temperature was measured every hour. Data available upon request.  

quarterHour_condensed <- readRDS("quarterHour_condensed.rds") #Matrix of 15 min temperatures
daily_means_condensed <- readRDS("daily_means_condensed.rds") #Matrix of mean daily water temperatures (dataset available upon request)
daily_maxs_condensed <- readRDS("daily_maxs_condensed.rds") #Matrix of daily maximum temperatures (dataset available upon request)
daily_mins_condensed <- readRDS("daily_mins_condensed.rds") #Matrix of daily minimum temperatures (dataset available upon request)
...
##Historical temperature data from Alexander and Stender Ponds was provided by the Mid Klamath Watershed Council (MKWC). Temperature data were collected hourly in these ponds from 2010-2021 using HOBO U22 sensors. Data available upon request.

APH <- read.csv("Alexander_Historical_2.csv") #Historical data from Alexander Pond (dataset available upon request)
SPH <- read.csv("Stender_Historical_2.csv") #Historical data from Stender Pond (dataset available upon request)
...
```{r}
z_daily_means_condensed <- zscore(daily_means_condensed) #Dataset available upon request
```
...
```{r}
#Read in data
daily_maxs_condensed <- readRDS('daily_maxs_condensed.RDS') #Dataset available upon request
...
```{r}
#Read in data
daily_mins_condensed <- readRDS('daily_mins_condensed.RDS') #Dataset available upon request
...
APH <- read.csv("Alexander_Historical_2.csv") #Dataset available upon request
...
APH_ts2 <- ts(APH$Temp, start = c(351, 13), frequency = 24) 
...
###Wavelet
```{r}
APH.w <- analyze.wavelet(y_inter3_df, "temp",
                        loess.span = 0,
                        dt = 1/24, 
...
power_day <- cbind(power_day, y_inter3_df$date)
...
SPH <- read.csv("Stender_Historical_2.csv") #Dataset available upon request
...
SPH_ts2 <- ts(SPH$Temp, start = c(351, 13), frequency = 24) 
...
```{r}
#run the wavelet
SPH.w <- analyze.wavelet(y_inter4_df, "temp",
                        loess.span = 0,
                        dt = 1/24, 
                        make.pval = TRUE, n.sim = 1000,
                        date.format = "%Y-%m-%d-%h") 
...
power_day2 <- cbind(power_day2, y_inter4_df$date)
